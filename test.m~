...
theta = [rand(n,1); 5e-2*reshape(eye(n),n*n,1)];
num_nodes_nn = fix(n);
%w1 = [1e15*rand(1,num_nodes_nn); rand(1,num_nodes_nn)];
w1 = 1e15*rand(1,num_nodes_nn);
w2 = 1e-22*rand(num_nodes_nn,n);
b1 = 1e15;
b2 = 1e-22*ones(num_nodes_nn,1);
...
for t=1:T
    ...
    nabla_g_y_theta_g = exp(log(y)+log_nabla_g1_theta_temp)*temp-nabla_g2_theta;
    ...
    %else
        essential_temp = exp(log_p_alpha_v_w + log(1+exp(-log(y)-log_p_alpha_v_w)));
    %end
